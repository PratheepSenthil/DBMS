...
rem * 4 Display the laureates name with their age at the time of award for the 
rem     Peace category.
...
rem * 5 Display the laureates (name,category,aff_role,country) whose name starts
rem     with A or ends with a, but not from Isreal.

select name,category,aff_role,country 
from nobel 
where (name like 'A%' or name like '%a') and country!='Isreal';

rem * 6 Display the name, gender, affiliation, dob and country of laureates who 
rem     was born in 1950s.Label the dob column as Born 1950.
...
rem * 8 Display the university name(s) that has to its credit by having at 
rem     least 2 nobel laureate with them.
...
rem    Label the column as Younger, Elder respectively. Include only the country 
rem    having more than one laureate. Sort the output in alphabetical order of country.
...
insert into nobel values(200,'Name1','m','Phy','Linear algebra',2001,'Anna University','29-dec-1999','India');
